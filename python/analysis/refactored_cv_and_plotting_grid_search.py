...
grid_search = GridSearchCV(classifier, param_grid=param_grid, scoring='roc_auc', cv=inner_cv)
...
for split, (train_index, test_index) in enumerate(data_split):
    ...
    auc_score_gridsearch = roc_auc_score(y_test, y_pred)
    print(f"AUC score for fold {split+1} with grid search: {auc_score_gridsearch}")
    continue
