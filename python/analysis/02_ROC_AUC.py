...
def Kfold_CV_solver(solver, X, y, groups, folds, stratified):
    ...
    X : DataFrame
        Vectoriced PSDs (features) of each subject_and_segment (observation).
    y : Series
...
if __name__ == "__main__":
    ...
    # Separate these for easier understanding
    X = dataframe.iloc[:,2:dataframe.shape[1]]
    y = dataframe.loc[:, 'Group']
    group = dataframe.loc[:, 'Subject']
    
    # TODO: Rename this variable save_file
#    save_file = f"{figures_dir}{args.clf}_{args.task}.pdf"
   
#    fig, ax = plt.subplots() #TODO: should these be given for the functions?heikkiv/work_s2022/mtbi-eeg
    
    if CV:
        # TODO: Maybe use a dict here?
        test_index, pred, tprs, aucs, mean_fpr, average_accuracy, accuracy_std, AUC, params = Kfold_CV_solver(solver=args.clf, X=X, y=y, groups=group, folds=10, stratified=True)
#Muting these for debugging purposes
...
#        plt.savefig(fname=f"{figures_dir}loo-ROC_{args.clf}_{args.task}.pdf")
